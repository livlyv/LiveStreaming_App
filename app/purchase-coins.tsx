...
import { ArrowLeft, Coins, Clock } from "lucide-react-native";

interface CoinPackage { id: string; coins: number; price: number; bonus: number }
  ...
  return (
    <View style={styles.container}>
      <SafeAreaView edges={["top"]} style={styles.safeArea}>
        ...
        <LinearGradient colors={["#E30CBD", "#6900D1"]} style={styles.balanceCard}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={styles.section}>
            ...
            {coinPackages.map((pkg) => (
              <TouchableOpacity key={pkg.id} style={styles.packageCard} onPress={() => handlePurchase(pkg)} disabled={loadingId === pkg.id}>
                ...
                <View style={styles.packageRight}>
                  ...
                  <Text style={styles.total}>Total: ₹{calculateTotal(pkg.price).toFixed(2)}</Text>
                </View>
              </TouchableOpacity>
            ))}
          </View>
            ...
            {transactions.length === 0 ? (
              ...
              transactions.slice(0, 10).map((transaction) => (
                <View key={transaction.id} style={styles.transactionCard}>
                  ...
                  <Text style={[styles.transactionAmount, transaction.amount > 0 ? styles.amountPositive : styles.amountNegative]}>
...
const styles = StyleSheet.create({
  ...
  balanceCard: { margin: 20, padding: 30, borderRadius: 20, alignItems: "center" },
  balanceLabel: { color: "#FFFFFF", fontSize: 16, marginTop: 10, opacity: 0.9 },
  balanceAmount: { color: "#FFFFFF", fontSize: 36, fontWeight: "bold", marginTop: 5 },
  conversionRate: { color: "#FFFFFF", fontSize: 14, marginTop: 10, opacity: 0.8 },
  section: { paddingHorizontal: 20, marginBottom: 30 },
  sectionTitle: { color: "#FFFFFF", fontSize: 18, fontWeight: "bold", marginBottom: 15 },
  packageCard: { flexDirection: "row", justifyContent: "space-between", backgroundColor: "rgba(255, 255, 255, 0.05)", padding: 20, borderRadius: 15, marginBottom: 10, borderWidth: 1, borderColor: "rgba(227, 12, 189, 0.2)" },
  packageLeft: { flex: 1 },
  coinAmount: { color: "#FFFFFF", fontSize: 18, fontWeight: "bold" },
  bonusText: { color: "#FFD700", fontSize: 14, marginTop: 5 },
  packageRight: { alignItems: "flex-end" },
  price: { color: "#FFFFFF", fontSize: 16 },
  gst: { color: "#999", fontSize: 12, marginTop: 2 },
  total: { color: "#E30CBD", fontSize: 14, fontWeight: "bold", marginTop: 5 },
  transactionCard: { flexDirection: "row", justifyContent: "space-between", backgroundColor: "rgba(255, 255, 255, 0.05)", padding: 15, borderRadius: 10, marginBottom: 10 },
  transactionLeft: { flex: 1 },
  transactionDesc: { color: "#FFFFFF", fontSize: 14 },
  transactionDate: { color: "#666", fontSize: 12, marginTop: 5 },
  transactionAmount: { fontSize: 16, fontWeight: "bold" },
  amountPositive: { color: "#4CAF50" },
  amountNegative: { color: "#FF5252" },
