-- UPDATED FINAL DATABASE SCHEMA FOR LIVE STREAMING APP
-- This schema includes ALL migrations from coins_earned to credits_earned
...
-- 1. Rename coins_earned to credits_earned in users table if it exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'coins_earned') THEN
...
-- 2. Rename coins_earned to credits_earned in streams table if it exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'streams' AND column_name = 'coins_earned') THEN
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'credits_earned') THEN
